...
pub mod get_path {
    use super::*;
    fn core(slf: &mut QFilePath) -> Result<PathBuf, Box<dyn Error>> {
        if cfg!(unix) {
    ...
    pub async fn async_get_path_buf(slf: &mut QFilePath) -> Result<PathBuf, Box<dyn Error>> {
        core(slf)
    }
    pub async fn async_get_path_string(slf: &mut QFilePath) -> Result<String, Box<dyn Error>> {
        Ok(core(slf)?.display().to_string())
    }
}
