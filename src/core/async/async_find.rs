...
use std::thread;
use walkdir::WalkDir;
pub async fn async_find_paths<T: AsRef<str> + Send + Sync + 'static>(
    place: Directory,
    name: T,
    follow_link: bool,
        ...
        async_std::task::block_on(async move {
            ...
            for element in paths {
                    ...
                    .filter_map(|e| e.ok())
                {
                    ...
                    {
                        sender.send(entry.path().to_path_buf().into()).await?;
                    }
                }
            }
            Ok::<(), SendError<PathBuf>>(())
        })
    });
    Ok(())
}
pub async fn async_find_regex_paths(
    place: Directory,
    name: Regex,
    follow_link: bool,
    sender: Sender<PathBuf>,
